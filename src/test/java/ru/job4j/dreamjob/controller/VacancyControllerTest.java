...
public class VacancyControllerTest {
    ...
    @Test
    public void whenRequestVacancyModifyThenGetVacancyPage() {
        var id = 1;
        var vacancyIdCaptor =  ArgumentCaptor.forClass(int.class);
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        when(vacancyService.findById(vacancyIdCaptor.capture())).thenReturn(Optional.of(vacancy));
        ...
        var actualId = vacancyIdCaptor.getValue();
        var actualCities = model.getAttribute("cities");
    ...
    @Test
    public void whenRequestVacancyModifyThenGetErrorPageWithMessage() {
        when(vacancyService.findById(any(Integer.class))).thenReturn(Optional.empty());
        var messageException = "Вакансия с указанным идентификатором не найдена.";

        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, 1);
        var actualMessageException = model.getAttribute("message");

        assertThat(view).isEqualTo("errors/404");
        assertThat(actualMessageException).isEqualTo(messageException);
    }

    @Test
    public void whenPostUpdateVacancyWithFileThenRedirectToVacanciesPage() throws Exception {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenPostUpdateVacancyAndSomeExceptionThrown() {
    ...
    @Test
    public void whenPostUpdateVacancyThenGetErrorPageWithMessage() {
    ...
    public void whenDeleteVacancyThenRedirectToVacanciesPage() {
        var id = 1;
        var vacancyIdCaptor = ArgumentCaptor.forClass(int.class);
        when(vacancyService.deleteById(vacancyIdCaptor.capture())).thenReturn(true);
    ...
    public void whenDeleteVacancyThenGetErrorPageWithMessage() {
        var expectedException = "Вакансия с указанным идентификатором не найдена";
        when(vacancyService.deleteById(any(int.class))).thenReturn(false);
